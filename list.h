...
#define list_for_each_entry_reverse(pos, head, member)				     \
	for (typeof(pos)LOCAL(p) = (pos = list_entry((head)->n.prev,		     \
			typeof(*pos),						     \
			member),						     \
			...
			member));						     \
			&pos->member != &(head)->n;				     \
			pos = LOCAL(p), LOCAL(p) = list_entry(LOCAL(p)->member.prev, \
			typeof(*LOCAL(p)),					     \
			member))
...
static inline void list_safe_add_tail(struct list_node *new, struct list_head *head)
{
	if (list_linked(new) && !list_empty((struct list_head *)new)) {
		list_del(new);
	}
